...
class TestSlotFillingConfirmation:
    ...
    async def test_basic_info_correction_flow(self, mock_services):
        ...
        with patch('app.services.rag_service.rag_service', mock_rag), \
            ...
            result_stream = []
            async for item in run_agent_streaming(
                user_input_text="아니요, 이름은 김철수이고 번호는 010-9876-5432입니다",
                session_id=session_id,
            ...
            # 검증: 정보가 수정되었는지
            collected_info = final_state.get("collected_product_info", {})
            assert collected_info.get("customer_name") == "김철수"
            assert collected_info.get("customer_phone") == "010-9876-5432"
            
            # 상태가 여전히 greeting 단계에 있는지 (재확인을 위해)
            assert final_state["current_scenario_stage_id"] == "greeting"
            
            print(f"✅ 정보 수정 완료: {collected_info}")
    ...
    async def test_internet_banking_grouped_collection(self, mock_services):
        ...
        with patch('app.services.rag_service.rag_service', mock_rag), \
            ...
            # 다음 단계로 진행되었는지 확인
            # (현재 구현에서는 process_internet_banking_info로 이동)
            assert final_state["current_scenario_stage_id"] == "process_internet_banking_info"
    ...
    async def test_state_transitions_tracking(self, mock_services):
        ...
        with patch('app.services.rag_service.rag_service', mock_rag), \
            ...
            async for item in run_agent_streaming(
                ...
                if isinstance(item, dict) and item.get("type") == "state_update":
                    state_transitions.append({
                        "stage": item["data"].get("current_scenario_stage_id"),
                        "product": item["data"].get("current_product_type")
                    })
            ...
            # 제품 선택이 발생했는지
            product_selected = any(t["product"] == "deposit_account" for t in state_transitions)
            assert product_selected
            
            # greeting 단계로 전환되었는지
            greeting_reached = any(t["stage"] == "greeting" for t in state_transitions)
            assert greeting_reached
            
            print(f"✅ 상태 전환 추적 완료: {len(state_transitions)} 전환 발생")
            for i, transition in enumerate(state_transitions):
                print(f"  {i+1}. Stage: {transition['stage']}, Product: {transition['product']}")
    ...
    async def test_slot_filling_updates(self, mock_services):
        ...
        with patch('app.services.rag_service.rag_service', mock_rag), \
            ...
            async for item in run_agent_streaming(
                ...
                if isinstance(item, dict) and item.get("type") == "state_update":
                    collected_info = item["data"].get("collected_product_info", {})
                    if collected_info:
                        slot_updates.append(collected_info.copy())
            
            # 슬롯 업데이트 검증
            assert len(slot_updates) > 0
            
            # 마지막 업데이트에서 평생계좌 정보가 추가되었는지
            final_slots = slot_updates[-1]
            assert "use_lifelong_account" in final_slots
            assert final_slots["use_lifelong_account"] is True
            
            # 기존 정보가 유지되는지
            assert final_slots.get("customer_name") == "홍길동"
            assert final_slots.get("customer_phone") == "010-1234-5678"
            assert final_slots.get("confirm_personal_info") is True
            
            print(f"✅ 슬롯필링 업데이트 추적 완료: {len(slot_updates)} 업데이트 발생")
            print(f"최종 슬롯 상태: {final_slots}")
